...
const filterWords = async (
  ...
  try {
    // Open and read file
    const wordsFolderPath = path.join(__dirname, "../data/");
    ...
    for await (const l of rl) {
      ...
      if (absentLettersSet && !absentLetterFilter(line, absentLettersSet)) {
        continue;
      }
      if (lettersSet && !lettersFilter(line, lettersSet)) {
        continue;
      }
      if (pattern) {
    ...
    return "Error: could not filter words";
  }
};
...
const lettersFilter = (
  word: string,
  lettersSet: Record<string, number>
): boolean => {
